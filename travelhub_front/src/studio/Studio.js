...
function move_cal(start_p, end_p, deg) {
  //카메라 움직임 도와주는 함수 스타트지점 끝지점 정도 deg는 0~1값
...
function addImagePlane(
  ...
  textureLoader.load(imagePath, function (texture) {
    ...
    const textGeometry = new TextGeometry("내 여행 사진들\n설명\n입니다.", {
      font: font,
      size: 0.2,
      height: 0,
      curveSegments: 12,
...
function addFrame(scene, width, height, position, rotation) {
  ...
  const frameMeshes = frameGeometries.map(
    (geometry) => new THREE.Mesh(geometry, frameMaterial)
  );

  // 프레임 위치 설정
  frameMeshes[0].position.set(
    position.x,
    position.y + height / 2 + frameThickness / 2,
    position.z
  ); // 상단 프레임
  frameMeshes[1].position.set(
    position.x,
    position.y - height / 2 - frameThickness / 2,
    position.z
  ); // 하단 프레임
  if (rotation.y === -1.5708 || rotation.y === -4.7124) {
    frameMeshes[2].position.set(
      position.x,
      position.y,
      position.z - width / 2 - frameThickness / 2
    ); // 좌측 프레임
    frameMeshes[3].position.set(
      position.x,
      position.y,
      position.z + width / 2 + frameThickness / 2
    ); // 우측 프레임
    ...
    frameMeshes[2].position.set(
      position.x - width / 2 - frameThickness / 2,
      position.y,
      position.z
    ); // 좌측 프레임
    frameMeshes[3].position.set(
...
const Studio = () => {
  ...
  useEffect(() => {
    ...
    const columns = [
      ...
      {
        position: new THREE.Vector3(40, 1, -5),
        width: 2,
        height: 40,
        depth: 1,
      },
    ...
    if (isCameraMoved) {
      ...
      camera.quaternion.setFromEuler(
        new THREE.Euler(-0.3, (Math.PI / 2 / 90) * 45, 0.3)
      );
    } else {
    }
    ...
    if (mapJson) {
      for (const mapDataNum in mapJson.defualt) {
        ...
        if (frame.direction === "North") {
          imagecoord = new THREE.Vector3(
            frame.coordinates[0] - 2,
            frame.coordinates[1],
            frame.coordinates[2] - 4
          );
        } else if (frame.direction === "East") {
          imagecoord = new THREE.Vector3(
            frame.coordinates[0] + 4,
            frame.coordinates[1],
            frame.coordinates[2] - 2
          );
    ...
    const handleWheel = (event) => {
      ...
      camera.position.x = move_cal(
        mapJson.defualt[floor].coordinates[0],
        mapJson.defualt[next_floor].coordinates[0],
        movingCircle - floor
      );
      camera.position.y = move_cal(
        mapJson.defualt[floor].coordinates[1],
        mapJson.defualt[next_floor].coordinates[1],
        movingCircle - floor
      );
      camera.position.z = move_cal(
        mapJson.defualt[floor].coordinates[2],
        mapJson.defualt[next_floor].coordinates[2],
        movingCircle - floor
      );
      camera.rotation.y =
        camera.rotation.y -
        delta *
          Math.PI *
          directionValues[
            mapJson.defualt[floor].direction +
              mapJson.defualt[next_floor].direction
          ];
      ...
      if (movingCircle < 0) {
        ...
        if (movingFlow > 40) {
          movingFlow =
            movingFlow -
            (movingFlow % 42) +
            42 * (Math.round(Number(movingFlow % 42)) !== 0);
        }
      }
  ...
  return (
    ...
    >
      <button
        ...
        onClick={handleCameraPositionToggle}
      >
        카메라 위치 토글
      </button>{" "}
      {/* 버튼 추가 */}
